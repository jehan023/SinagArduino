...
String GAS_ID = "AKfycbyi-dKJaoBybaowvOy-e6M3A2ome2OscSD5n3mKq6f5DQfBerZg2uCQJL0ZixHbN6tF";  // GSheet Script Deployment ID
...
bool isLoRaWorking() {
  // Check LoRa module functionality by sending a test packet
  if (LoRa.beginPacket()) {
    LoRa.endPacket();
    return true;
  } else {
...
void sendMessage(String outgoing, byte MasterNode, byte otherNode) {
  if (!isLoRaWorking()) {
    // LoRa module is not working, reconnect
    Serial.println("LoRa module is not working. Reconnecting...");
    initLORA();
  } else {
...
void saveToCloud(String node, float pvv, float pvp, float bv, float bl, float bp, float ls, float l, float t, float snr, float rssi, String dt, float ch) {
  ...
  //   saveToSD(node, pvv, pvp, bv, bl, bp, ls, l, t, snr, rssi, ch);
  //   return;
  // }
